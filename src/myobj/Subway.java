package myobj;

public class Subway {
    public String[] stations;
    
   
    
	public int line;
	public int current_station;
	public boolean direction;

	//생성자 (클래스 이름과 같아야 함) 이걸지우면 OOP subway에 기본값을 입력해야해서 에러납니다.
	/*public Subway(boolean direction, boolean rotate) {
		
	}*/

	public Subway(int line) {
		this.line = line;
	}
	
	//열차 순환을 나타내는것
	public boolean rotate;
	
	public void next() {
		current_station = rotate ? nextRotateStation() : nextStation();
	} 
	
	
	// 인스턴스 메서드를 만들 때는 static을 사용하지 않는다.
/*	public void next() {
		current_station += 1;
	} 이러면 2호선이나 역마지막일때 에러가남~) 그래서 나온게 */
	
 	//열차의 현재 위치를 실제로 다음역으로 변경하는 메서드
	public void next1() {
		current_station = nextStation();
	}

 	// 다음역이 무엇인지 계산하여 반환하는 메서드
	public int nextStation() {
	
		
		if (current_station == stations.length - 1) {
			direction = false;
			
		}else if (current_station == 0) {
			direction = true;
		}	
		return current_station + (direction? 1 : -1);
	}
	
	
	
	public int nextRotateStation() {
		if (direction && current_station == stations.length - 1) {
			return 0;
		} else if (!direction && current_station == 0) {
			return stations.length -1;
		}
		return current_station + (direction ? 1 : -1 );
	}
	


	
	public void informationbase() {
		System.out.println("-------------------------");
		System.out.printf("이 열차는 %d호선 입니다.\n",line);
		System.out.printf("현재 역은 %s역 입니다.\n", stations[current_station]);
		System.out.printf("다음 역은 %s역 입니다.\n", stations[nextStation()]);
		System.out.println("-------------------------");
	}
	
	public void information() {
		System.out.println("-------------------------");
		System.out.printf("이 열차는 %d호선 입니다.\n",line);
		System.out.printf("현재 역은 %s역 입니다.\n", stations[current_station]);
		System.out.printf("다음 역은 %s역 입니다.\n", stations[direction ? nextRotateStation() : nextStation()]);
		System.out.println("-------------------------");
	}
	
	
	
	
	
	

}
