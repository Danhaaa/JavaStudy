...
public class E05_CharacterStream {
	/*
	 * # InputStream / OutputStream - byte 단위로 데이터를 입/출력하는 클래스 - byte로 분해되어 있는 데이터를
	 * 다른 타입으로 직접 변화시켜야 해서 불편함
	 * 
	 * # Reader / Writer - char 단위로 데이터를 입/출력하는 클래스 - byte를 char로 합치는 기능과 char를
	 * byte로 분해하는 기능이 구현되어 있다
	 * 
	 * # Charset - ASCII코드까지는 모든 Charset이 동일하지만 그 이후부터는 각 회사마다 문자코드가 다르다 - MS949 :
	 * 윈도우즈에서 사용하는 마이크로소프트의 문자셋 - EUC-KR : 이클립스가 기본적으로 사용하는 문자셋 (유효한 한국어만 사용, 뙗뺡 x)
	 * - UTF-8 : 가장 표준이 되는 문자셋으로 모든 문자를 포함하고 있어서 용량이 크다
	 */

	public static void main(String[] args) {
		byte[] data1 = "Hello!!".getBytes(); // [72, 101, 108, 108, 111, 33, 33]
		byte[] data2 = null;
		byte[] data3 = null;
		byte[] data4 = null;
		...
		try {
			...
			// ASCII 코드 까지는 인코딩 타입(Charset)이 필요없다
		...
		// byte[]을 다시 합칠 때도 분해했을 때 사용했던 타입을 사용해야 제대로 합칠 수 있다
		try {
		...
		try {
			FileReader in = new FileReader("myFiles/writer1.txt", 
